...
{
    /// <summary>
    /// Interaction logic for FindResFromOrgThatNotInSus.xaml
    /// </summary>
    public partial class FindResFromOrgThatNotInSus : Window
    {
        // === Variables area ===
        ...
        // === === === === === === 
        public FindResFromOrgThatNotInSus()
        {
            InitializeComponent();
            ButtonActions(false);
        }
    ...
    $"You have selected {openFileDialog.SafeFileName} and we found {excelReaderResult.Count} rows in the file";
        ...
        private void Start_Op(object sender, RoutedEventArgs e)
        {
            ...
            {
                foreach (var res in _orgDataList)
                {
                    if (_susDataList.FirstOrDefault(a => a.Name == res.Name) != null)
                    {
                        continue;
                    }
                ...
                _susDataList.AddRange(listOfResourcesFound);
                _susDataList = _susDataList.OrderBy(a => a.Name).ToList();
                ...
                switch (userReaction)
                {
                    case MessageBoxResult.Yes:
                        ...
                        var newSusResult = SaveAsExcel(
                            ConvertJsonToDataTable(_susDataList.ToJson()),
                            location + "-Merged").Result;
                        OpTextBox.Text = $"Missing rows Saved : {rows} \n" +
                                         $"Missing rows Added to Sus file and Merged file saved : {newSusResult} \n" +
                                         $"Done";
                        ButtonActions(false);
                        break;
        ...
        private string SaveDialog()
        {
            ...
            saveFileDialog.Title = "یجا درست درمون سیو کن بی صاحابو";
